...
contract CoWSwapProviderTest is Test {
    ...
    function test_cancelTrade_RevokesSignatureAndReturnsFunds() public {
        uint32 validTo = uint32(block.timestamp + 3600);
        bytes32 digest = _tradeDigest(validTo);
        assertEq(provider.isValidSignature(digest, ""), MAGICVALUE, "order active before cancel");

        vm.prank(owner);
        provider.cancelTrade(abi.encode(digest, validTo));

        assertEq(provider.isValidSignature(digest, ""), bytes4(0xffffffff), "order revoked after cancel");
        assertEq(usdc.balanceOf(owner), 1000e6, "sell tokens returned");
